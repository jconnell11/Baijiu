...
if len(sys.argv) > 1:
  lib = CDLL("./DLL/" + sys.argv[1] + ".dll")
else:
  lib = CDLL("./DLL/baijiu_test.dll")
...
# play error sound (blocks)

def fail_sound():
  os.system("powershell -c (New-Object Media.SoundPlayer './sfx/squawk.wav').PlaySync();")
      ...
      if lib.ext_start(c_char_p(name[15:20].encode()), dbg) <= 0:
        ...
        while True:
          if stop > 0:
            ...
            break
          if (time.time() - last) > 2:
            print("\n\x1b[1;33m  >>> Link: Bluetooth connection lost!\x1b[0m")
            fail_sound()
            break
          time.sleep(0.1)                
  except:
