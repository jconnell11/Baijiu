...
int jhcQtCamCal::Launch (int dbg)
{
  // connect to ESP32Cam and set checkerboard unwarping values
  if (ocv_open(ipcam, 1) < 0)                   
  {
    printf("\x1b[1;33m  >>> No active camera found!\x1b[0m\n");
    return 0;
  }
  ocv_warp(0.14, -0.13, 0.024, 219, 1, 313, 242);    

  // make a window to display video
  ocv_win(0, "Marked Points", 0, 0);
...
void jhcQtCamCal::draw_cross (int x, int y)
{
  ...
  // horizontal line 
  d = (unsigned char *) img + 1920 * by;         // override const
  x0 *= 3;
  x1 *= 3;
  for (i = x0; i < x1; i += 3)
  {
  ...
  // vertical line
  d = (unsigned char *) img + 3 * x;             // override const
  y0 *= 1920;
  y1 *= 1920;
...
int jhcQtCamCal::compute_calib ()
{
  ...
  double hx, hy, hz, cx, cy, cz, th, pc, tc, rc, ti;
  ...
  CamDir(pc, tc, rc);                            // includes original ct0
  ct0 += th - (tc + ti);               
  ...
  printf("\nHit any key to save values to file (ESC to abort) ...\n");
  step++;
  return 1;
}
...
int jhcQtCamCal::save_values ()
{
  ...
  // write previous name, IP, and servo offsets plus new camera values 
  fprintf(out, "%s\n", name);
  fprintf(out, "%s\n", ipcam);
