...
protected:
  // Microbit ID, robot name, and video URL
  char mb[10], name[40], ipcam[40];

  // arm geometry
  double by, bs, sz, sw, aim, boff, soff, goff;
...
public:
  ...
  int BluStart (const char *id, int dbg);
...
protected:
  ...
  void LookAt (double x, double y, double z, double dps =90.0);
  void CamLoc (double& x, double& y, double& z) const;
  void CamDir (double& p, double& t, double& r) const;
  ...
  void HandDir (double& p, double& t, double& r) const;
  ...
  // utilities
  void Pace (int ms =33);
  void PlaySFX (const char *snd) const;
  void Show () const;
  void XYZ () const;
  void Stop ();
