...
void jhcQtruck::cfg_params ()
{
  ...
  aim = 15.0;                // camera wrt shoulder-wrist (deg)
...
int jhcQtruck::BluStart (const char *id, int dbg)
{
  // initialize main system
  calib_vals(id);
  def_vals();
  if (Launch(dbg) <= 0)                   // override                   
    return 0;
...
//   line 2 = IP address          full camera streaming URL     
//   line 3 = boff soff goff      arm servo offsets for zero angle
//   line 4 = cp0 ct0 cr0         camera angle adjustments wrt nominal
...
void jhcQtruck::calib_vals (const char *id)
{
  ...
  // look for calibration file associated with micro:bit ID
  strcpy_s(mb, id);
  ...
  // read file contents (with early termination option)
  while (1)
  {
    // robot name
    ...
    // streaming URL
    if (fgets(ipcam, 40, in) == NULL)
      break;
    ipcam[strlen(ipcam) - 1] = '\0';
...
void jhcQtruck::calc_speeds ()
{
  ...
  dps0 = lag * scrub * R2D * (rsp - lsp) / tsep;
}
...
//= Attempt to center camera view near location (x y z).
// y points forward, x is to right, z up from floor, coordinates in inches
// <pre>
//            cam         C = angle of sh-cam link wrt sh-wrist    
//             +              
//       sc  / T  .           T = cam axis wrt sh-cam direct link  
//         /        .             
//       /  B         . hyp       d^2 = sc^2 + hyp^2 - 2 * sc * hyp * cos(T)
//  sh + _              .            
//     | A   -  _  d      .           hyp / sin(B) = d / sin(T)
//  dz |            -   _   .          
//     |                   -  .           st = (A + B - C) - 90
//     +-----------------------+  obj        
//               dr
...
//d^2 = sc^2 + hyp^2 - 2 sc hyp cos(T)

void jhcQtruck::LookAt (double x, double y, double z, double dps)
{
  double sc, C, T, dx, dy, dz, dr, A, d, b, c, hyp, B;

  // camera look-down wrt shoulder-camera link
  sc = sqrt(cdot * cdot + crt * crt);
  C = R2D * atan2(crt, cdot);
  T = 180.0 - (C + aim - ct0);

  // object wrt arm rotation axis
  dx = x;
  dy = y - by;
  dz = sz - z;
  dr = sqrt(dx * dx + dy * dy);  

  // lower shoulder-object triangle
  A = R2D * atan2(dr, dz);
  d = sqrt(dz * dz + dr * dr);

  // upper shoulder-camera triangle
  // hyp^2 + [-2 * sc * cos(T)] * hyp + (sc^2 - d^2) = 0
  b = -2.0 * sc * cos(D2R * T);
  c = sc * sc - d * d;
  hyp = 0.5 * (-b + sqrt(b * b - 4 * c));
  B = R2D * asin(hyp * sin(D2R * T) / d);

  // desired canonical servo angles
  st = A + B - C - 90.0;
  bt = R2D * atan2(-dx, dy) - cp0;
  asp = dps;
}
...
// pan is CCW from RIGHT (fwd = 90), tilt is up from horizontal, angles in degrees

void jhcQtruck::HandDir (double& p, double& t, double& r) const
{
  p = bnow + 90.0;
  t = 0.0;                   // could add calibration offset
  r = 0.0;
}
...
void jhcQtruck::Grip (double w, double ips)
{
  ...
  gt = R2D * asin(s);                  // g = 0 is straight out
  gsp = R2D * 0.5 * ips / jaw;         // approximate
}
...
{
  return(fsep + 2.0 * jaw * sin(D2R * gnow));
}
...
{
  double pct, v100 = 4.0, v20 = 3.5, v10 = 3.2, v0 = 3.0;  
...
void jhcQtruck::PlaySFX (const char *snd) const
{
  wchar_t wname[80];
  char fname[80];
  
  sprintf_s(fname, "sfx/%s.wav", snd);
  mbstowcs_s(NULL, wname, fname, 80);
  PlaySound(wname, NULL, SND_SYNC);
}
...
void jhcQtruck::Show () const
{
  ...
  cmd2[0]  = cmd[0];         // left track
  cmd2[1]  = cmd[1];
  cmd2[2]  = sep;
  cmd2[3]  = cmd[2];         // right track
  cmd2[4]  = cmd[3];
  cmd2[5]  = sep;
  cmd2[6]  = cmd[4];         // shoulder rotation
  cmd2[7]  = cmd[5];
  cmd2[8]  = cmd[6];
  cmd2[9]  = sep;
  cmd2[10] = cmd[7];         // shoulder lift
  cmd2[11] = cmd[8];
  cmd2[12] = sep;
  cmd2[13] = cmd[9];         // gripper width
  cmd2[14] = cmd[10];
  cmd2[15] = sep;
  cmd2[16] = cmd[11];        // LED color
  cmd2[17] = sep;
  cmd2[18] = cmd[12];        // mouth state
  cmd2[19] = '\0';
  printf("  head %5.1f, tilt %3.0f, roll %3.0f, dist %5.1f, line %d%d%d%d, bat %4.2f, servo[%3.0f %3.0f %3.0f] -> cmd \"%s\"\r",
         head, tilt, roll, dist, line >> 3, (line >> 2) & 1, (line >> 1) & 1, line & 1, volt, bnow, snow, gnow + g0, cmd2);
}
...
{
  double hx, hy, hz, hp, ht, hr, w, cx, cy, cz, cp, ct, cr;
  ...
  printf("    [%3.0f %3.0f %3.0f] --(%4.0f)--> hand [%3.0f %3.0f %3.0f] x %3.0f, cam [%3.0f %3.0f %3.0f] up %3.0f\r", 
         bnow, snow, gnow, hp, hx, hy, hz, w, cx, cy, cz, ct);
}
