...
extern "C" DEXP int spio_start (const char *path, int prog)
{
  ...
  // ---------------------------------------------------------------------------
  // CALLBACK: for general cancellation errors
  svc->Canceled.Connect([] (const SpeechRecognitionCanceledEventArgs& e)
  {
    reco = -2;                                           // lost network
  });
  ...
  Sleep(1500);                                             // wait for errors
  return reco;
}

  
//= Add proper spellings of words from file: <path>/config/all_names.txt.
// returns number of spellings added, negative for some error
// NOTE: meant to be called only once at beginning of program

extern "C" DEXP int reco_spell (const char *path)
{
  char fn[200], name[80];
  FILE *in;
  int n, cnt = 0;
...
extern "C" DEXP int tts_say (const char *msg, int pitch, int rate)
{
  WCHAR wide[200];
  char tag[40], cmd[200] = "";
  ...
  // add pitch and rate tags if needed
  if (pitch != 0)
    sprintf_s(cmd, "<pitch absmiddle=\"%d\">", __max(-10, __min(pitch, 10)));
  if (rate != 0)
  {
    sprintf_s(tag, "<rate absspeed=\"%d\">", __max(-10, __min(rate, 10)));
    strcat_s(cmd, tag);
  }
  strcat_s(cmd, msg);
  if (rate != 0)
  ...
  MultiByteToWideChar(CP_UTF8, 0, cmd, -1, wide, 200);
